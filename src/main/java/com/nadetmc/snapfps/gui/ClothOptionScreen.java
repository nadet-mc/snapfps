...
public class ClothOptionScreen {
    public static Screen generateScreen(Screen parent) {
        ...
        builder.setTitle(Text.translatable("text.snapfps.options.title"));
        ...
        general.addEntry(entryBuilder.startBooleanToggle(Text.translatable("text.snapfps.options.enabled"), config.enabled)
                .setDefaultValue(configDefaults.enabled)
                .setSaveConsumer(newValue -> config.enabled = newValue)
                .build());

        general.addEntry(entryBuilder.startBooleanToggle(Text.translatable("text.snapfps.options.advancedStats"), config.advancedStats)
                .setDefaultValue(configDefaults.advancedStats)
                .setSaveConsumer(newValue -> config.advancedStats = newValue)
                .build());

        general.addEntry(entryBuilder.startFloatField(Text.translatable("text.snapfps.options.textSize"), config.textSize)
        ...
        general.addEntry(entryBuilder.startColorField(Text.translatable("text.fpsdisplay.options.textColor"), config.textColor)
                ...
                .build());
        general.addEntry(entryBuilder.startIntSlider(Text.translatable("text.snapfps.options.textAlpha"), config.textAlpha, 0, 255)
                .setDefaultValue(configDefaults.textAlpha)
                .setSaveConsumer(newValue -> config.textAlpha = newValue)
                .build());
        general.addEntry(entryBuilder.startBooleanToggle(Text.translatable("text.snapfps.options.textShadows"), config.textShadows)
                .setDefaultValue(configDefaults.textShadows)
                .setSaveConsumer(newValue -> config.textShadows = newValue)
                .build());

        general.addEntry(entryBuilder.startIntField(Text.translatable("text.snapfps.options.offsetTop"), config.offsetTop)
                .setDefaultValue(configDefaults.offsetTop)
                .setSaveConsumer(newValue -> config.offsetTop = newValue)
                .build());
        general.addEntry(entryBuilder.startIntField(Text.translatable("text.snapfps.options.offsetLeft"), config.offsetLeft)
                .setDefaultValue(configDefaults.offsetLeft)
                .setSaveConsumer(newValue -> config.offsetLeft = newValue)
                .build());

        general.addEntry(entryBuilder.startEnumSelector(Text.translatable("text.snapfps.options.keybindMode"), ConfigData.KeyMode.class, config.keybindMode)
